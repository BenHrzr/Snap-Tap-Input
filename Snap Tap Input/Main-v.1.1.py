...
def start_programm():
    ...
    global running
    running = True
    StartButton.configure(background='white')
    StoppButton.configure(background='#B0E0E6')
    main()
...
def stopp_programm ():
    ...
    running = False
    keyboard.unhook_all()
    StoppButton.configure(background='white')
    StartButton.configure(background='#B0E0E6')
...
# 1. sprache geändert
# 3. logo hinzugefügt 
# 4. interaktive farbe des knopfes
# 5. curses verändert
